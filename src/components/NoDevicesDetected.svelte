...
<div class="flex flex-col justify-center items-center w-full h-full text-center text-neutral-300">
	<div class="w-80 text-sm">
		<h2 class="text-lg font-bold mb-2">{$t("nodevice.nodevice")}</h2>
		<p class="mb-2">{$t("nodevice.connection")}</p>
		{#if buildInfo?.split("</summary>")[0]?.includes("linux")}
			<p class="mb-2">{$t("nodevice.udev")}</p>
		{/if}
		<p class="mb-4">{$t("nodevice.plugin")}</p>
		<button
		...
		>
			{$t("nodevice.restart").replaceAll("{PRODUCT_NAME}", PRODUCT_NAME)}
		</button>
	</div>
</div>
